...
# :class: tip
# Vaikka funktio f(x) ei olisi määritelty kohdassa $x = x_0$, on mahdollista, että funktion arvot lähenevät rajatta tiettyä äärellistä arvoa **a**, kun muuttujan x:n arvot lähestyvät arvoa $x_0$. Tällöin lukua a kutsutaan funktion f **raja-arvoksi**, kun x lähestyy arvoa $x_0$.   Tällöin käytetään seuraavaa merkintää:      
# 
...
# Esim. funktiota $y = \frac {x^2 -1}{x-1}$ ei ole määritelty kohdassa x = 1. Kuvaajan perusteella funktion arvot lähestyvät molemmilta puolilta raja-arvoa 2 lähestyttäessä kohtaa x = 1.    
# 
...
# 
# Raja-arvo saadaan usein lasketuksi helposti sen jälkeen, kun nimittäjän ja osoittajan yhteinen tekijä on supistettu pois.

# ```{admonition} Laske lausekkeen $ \frac{3x^2+2x}{2x} $ raja-arvo kohdassa x = 0
# :class: dropdown
# Esitetään osoittaja tulomuodossa, ja supistetaan     
...
# Edelliset laskut voi kirjoittaa seuraavalla tavalla käyttämällä raja-arvomerkintää       
# 
